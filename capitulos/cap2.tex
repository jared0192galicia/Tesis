...
\subsection{Tecnologías modernas}
...
\subsubsection{Deno}
Deno es un runtime de JavaScript y TypeScript que se presenta como una alternativa moderna a Node.js, con un enfoque en la seguridad, la simplicidad y el soporte nativo para TypeScript. Su documentación oficial destaca características como la ejecución segura por defecto, la integración de herramientas de desarrollo y la compatibilidad con módulos ES \cite{deno2025docs}.
...
\subsubsection{Métodos de evaluación de usabilidad UI/UX}

\textbf{Pruebas de usabilidad con usuarios}
Consisten en la observación directa de usuarios representativos mientras realizan tareas específicas dentro de una interfaz. Este método permite identificar fricciones en el flujo de navegación y problemas de comprensión en la arquitectura de información, evaluando métricas como la tasa de éxito y el tiempo de ejecución \cite{bargas2023usability}.

\textbf{Evaluación heurística}
Es un método de inspección de usabilidad donde evaluadores expertos revisan la interfaz basándose en principios reconocidos, como las heurísticas de Nielsen. Su objetivo es detectar problemas de diseño mediante la comparación sistemática de la interfaz contra estándares de diseño interactivo \cite{molich2022heuristic}.

\textbf{Test A/B}
Es una técnica de experimentación cuantitativa que compara dos versiones de una misma interfaz. Al exponer a grupos distintos a variaciones específicas de diseño, se mide estadísticamente cuál versión optimiza los indicadores de rendimiento del sistema \cite{kohavi2023abtesting}.

\textbf{Pruebas de carga cognitiva (SUS)}
El System Usability Scale (SUS) es un cuestionario estandarizado que mide la percepción subjetiva de la facilidad de uso. A través de una escala tipo Likert, se cuantifica la satisfacción del usuario respecto a la complejidad y eficiencia percibida en la interacción con el sistema \cite{bangor2024sus}.
